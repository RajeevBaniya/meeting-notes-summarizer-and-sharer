...
function renderMarkdown(text) {
	...
	lines.forEach((line) => {
		...
		if (trimmed.startsWith('**') && trimmed.endsWith('**') && !trimmed.includes('*') || trimmed.match(/^\*\*[^*]+\*\*$/)) {
			...
			result.push(`<h3 class="font-semibold text-slate-100 mt-4 mb-2 text-base">${heading}</h3>`)
			return
		}
		...
		if (trimmed.startsWith('* ') && !trimmed.startsWith('**')) {
			...
			const processed = content.replace(/\*\*(.+?)\*\*/g, '<strong class="text-slate-200">$1</strong>')
